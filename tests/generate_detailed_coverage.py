...
def get_uncovered_lines(cls: ET.Element) -> str:
    uncovered = []
    ...
    for line in lines_el.findall("line"):
        if int(float(line.get("hits", 0))) == 0:
            ...
            if line_number is not None:
                uncovered.append(int(line_number))
...
def parse_complexity(element: ET.Element) -> float:
    return float(element.get("complexity", 0) or 0)


def generate_markdown_table(xml_file: str) -> None:
    ...
    total_complexity = parse_complexity(root)
    ...
    markdown += f"- **Overall Max Function Complexity:** `{total_complexity:.1f}`\n\n"

    markdown += "### 📄 Detailed Per-File Coverage\n\n"
    markdown += "| File | Line Coverage | Branch Coverage | Max Function Complexity | Uncovered Lines |\n"
    markdown += "| :--- | :---: | :---: | :---: | :--- |\n"
    ...
    for package in root.findall(".//package"):
        for cls in package.findall(".//class"):
            ...
            markdown += f"| {icon} `{filename}` | {line_pct} | {branch_pct} | {complexity_str} | `{uncovered_list}` |\n"
